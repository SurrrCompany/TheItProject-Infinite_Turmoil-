//Call:     inventory_equip(x,y,spr,alpha,backgroundcol)
//Info:     This will draw the inventory equip on the given x and y position. spr is
...
if (sprite_exists(spr_inv_back)) {
    draw_sprite_stretched_ext(spr,-1,xx,yy,(xx+207)-xx,(yy+240)-yy,c_white,0.5)
} else {
    draw_set_color(bgcol)
    draw_rectangle(xx,yy,xx+207,yy+240,0.5)
...
b=0
no=1
if (global.inv_optionshow=0) {global.inv_select=0}
repeat (7) {
    repeat (6) {
        var spr,index;
        spr=spr_items
        ...
        if ((global.inv_holditempos=no)=0) {
...
if (global.inv_holditem=true)
{
    var spr,index;
    spr=spr_basic_wpn
    index=global.inv_item[global.inv_holditempos,0]
    if (item_stackable(global.inv_item[global.inv_holditempos,0])=1)
    {spr=item_sprite(global.inv_item[global.inv_holditempos,0]); index=item_index(global.inv_item[global.inv_holditempos,1])}
    ...
    if (item_stackable(global.inv_item[global.inv_holditempos,0])=1)
    {
        ...
        draw_set_font(fnt002)
        draw_set_color(c_black)
        draw_text(mouse_x-15,mouse_y-15,value_str(global.inv_item[global.inv_holditempos,1]))
        draw_set_color(c_white)
        draw_text(mouse_x-16,mouse_y-16,value_str(global.inv_item[global.inv_holditempos,1]))
        draw_set_color(c_black)
        draw_set_alpha(1)
    }
}
